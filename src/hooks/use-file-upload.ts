...
export type FileUploadOptions = {
  ...
  autoUpload?: boolean; // Whether to automatically upload files when added
  onUploadProgress?: (fileId: string, progress: UploadProgress) => void; // Upload progress callback
  onUploadSuccess?: (fileId: string, response: UploadResponse) => void; // Upload success callback
  onUploadError?: (fileId: string, error: Error) => void; // Upload error callback
};
...
export type FileUploadActions = {
  ...
  uploadFile: (fileId: string) => Promise<UploadResponse>;
...
export const useFileUpload = (
  ...
  const uploadFileById = useCallback(
    async (fileId: string): Promise<UploadResponse> => {
      const fileToUpload = state.files.find((file) => file.id === fileId);
      if (!fileToUpload || !(fileToUpload.file instanceof File)) {
        throw new Error(
          `File with id ${fileId} not found or not a File object`
        );
      }
      ...
      setState((prev) => ({
        ...
        uploadProgress: {
          ...prev.uploadProgress,
          [fileId]: { loaded: 0, total: fileToUpload.file.size, percentage: 0 },
        },
      }));

      try {
        const response = await uploadFile(fileToUpload.file, (progress) => {
          setState((prev) => ({
            ...prev,
        ...
        options.onUploadError?.(fileId, uploadError);
        throw uploadError;
      } finally {
        setState((prev) => ({
          ...prev,
  ...
  const uploadAllFiles = useCallback(async (): Promise<UploadResponse[]> => {
    ...
    try {
      const responses = await uploadMultipleFiles(
        fileFiles.map((file) => file.file as File),
        (fileIndex, progress) => {
          const fileId = fileFiles[fileIndex].id;
          setState((prev) => ({
            ...prev,
            uploadProgress: { ...prev.uploadProgress, [fileId]: progress },
          }));
      ...
      fileFiles.forEach((file) => {
        options.onUploadError?.(file.id, uploadError);
      });
      throw uploadError;
    } finally {
